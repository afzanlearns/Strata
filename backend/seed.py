...
def num(v):
    """NaN -> None: SQLite tolerates NaN floats, MySQL/pymysql rejects them."""
...
def main() -> None:
    ...
    s.add_all([Station(
        station=r.station, state=r.state, district=r.district, lat=num(r.lat), lon=num(r.lon),
        label=r.label, median_depth=num(r.median_depth), p90_depth=num(r.p90_depth),
        annual_decline=num(r.annual_decline_m_per_year), monsoon_delta=num(r.monsoon_delta_m),
        volatility=num(r.volatility_m), gap_fraction=num(r.gap_fraction),
        quarantine_fraction=num(r.quarantine_fraction), kmeans=int(r.kmeans),
    ...
    s.add_all([District(
        ...
        median_depth=num(r.median_depth), annual_decline=num(r.annual_decline_m_per_year),
        frac_deepening=num(r.frac_deepening)) for r in dd.itertuples()])
    ...
    fc = pd.read_csv(f"{P}/forecasts_full.csv", parse_dates=["date"])
    s.add_all([Forecast(station=r.station, date=r.date.date(), actual=r.actual,
                        predicted=r.predicted) for r in fc.itertuples()])
